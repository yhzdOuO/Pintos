...
bool 
spt_hold_page(struct spt *spt, void *upage) {
    struct spte *spte = spt_get_spte(spt, upage);
    if (spte == NULL) {
        return false;
    }
    return bp_pin_page(spte->bp, spte);
}

void 
spt_release_page(struct spt *spt, void *upage) {
    struct spte *spte = spt_get_spte(spt, upage);
    if (spte == NULL) {
        return;
    }
    bp_unpin_page(spte->bp);
}

bool
spt_is_valid_range(struct spt *spt, void *start, size_t size, bool writable) {
    uintptr_t page;
    uintptr_t end = (uintptr_t) start + size - 1;

    for (page = (uintptr_t) pg_round_down(start); page <= end; page += PGSIZE) {
        if (!spt_is_valid_page(spt, (void *) page, writable)) {
            return false;
        }
    }
    return true;
...
bool 
spt_claim_range(struct spt *spt, void *start, size_t size) {
    uintptr_t page;
    uintptr_t end = (uintptr_t) start + size - 1;

    for (page = (uintptr_t) pg_round_down(start); page <= end; page += PGSIZE) {
        if (!spt_claim_page(spt, (void *) page)) {
            return false;
        }
    }
    return true;
...
bool
spt_hold_range(struct spt *spt, void *start, size_t size) {
    uintptr_t page;
    uintptr_t end = (uintptr_t) start + size - 1;

    for (page = (uintptr_t) pg_round_down(start); page <= end; page += PGSIZE) {
        if (!spt_hold_page(spt, (void *) page)) {
            return false;
        }
    }
    return true;
...
void
spt_release_range(struct spt *spt, void *start, size_t size) {
    uintptr_t page;
    uintptr_t end = (uintptr_t) start + size - 1;

    for (page = (uintptr_t) pg_round_down(start); page <= end; page += PGSIZE) {
        spt_release_page(spt, (void *) page);
    }
}
