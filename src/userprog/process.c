...
{
  struct process_start_args *start_args = start_args_;
  char *cmdline = start_args->cmdline;
  struct child_status *child_status = start_args->child_status;
  struct intr_frame if_;
  struct thread *cur = thread_current ();
  bool success = false;

  cur->self_status = child_status;
  free (start_args);

  #ifdef VM
  cur->spt = malloc (sizeof(struct spt));
  if (cur->spt == NULL) {
    success = false;
    goto VM_DONE;
  }
  spt_init (cur->spt, cur);
  cur->mmap_table = malloc (sizeof(struct mmap_table));
  if (cur->mmap_table == NULL) {
    success = false;
    goto VM_DONE;
  }
  mmap_table_init (cur->mmap_table, cur->spt);
  #endif
...
process_wait (tid_t child_tid) 
{
  ...
  // 真的有必要用waited吗？wait是串行的，怎么可能同时wait一个child两次？
...
child_status_release (struct child_status *status)
{
  ...
  ASSERT (status->ref_cnt > 0);
  status->ref_cnt--;
  should_free = status->ref_cnt == 0;
...
process_activate (void)
{
  ...
  /* Set thread's kernel stack for use in processing
     interrupts. */
  tss_update ();  // tss的设计是每个任务一个，现在只用于特权级切换的时候栈的切换，是每个cpu一个
}

...
load (char *cmdline, void (**eip) (void), void **esp) 
{
  ...
  argv_size = PGSIZE / 2 + 1;
  argv = malloc (sizeof *argv * argv_size);
  if (argv == NULL)
    goto done;
  ...
  if (!setup_stack (esp))
    goto done;
  if (!push_arguments (esp, argv, argc))
    goto done;
  ...
  while (read_bytes > 0 || zero_bytes > 0) 
    {
      ...
      struct spte_desc desc = {
        ...
        //.mmap = NULL,
        .kind = SPTE_FILE,
        .sharing = SPTE_PRIVATE,
        .file = file,
        .offset = ofs,
        .read_bytes = page_read_bytes,
        .zero_bytes = page_zero_bytes,
      };
      if (!spt_insert_spte(t->spt, &desc))
        return false;
...
setup_stack (void **esp) 
{
  ...
  if (!spt_insert_spte(thread_current()->spt, &desc) 
      || !spt_claim_page(thread_current()->spt, upage)) {
      return false;
    }
...
/* Pushes the command-line arguments described by ARGV and ARGC
   onto the initial user stack. */
static bool
