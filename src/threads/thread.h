...
struct thread
  {
    ...
    bool in_user_copy;                  /* Whether we're copying data to/from user. */
    uint8_t *user_stack;                 /* User stack pointer (for growth). */
#endif

#ifdef VM
    /* Owned by vm/page.c. */
    struct spt *spt;                     /* Supplemental page table. */
    struct mmap_table *mmap_table;       /* Memory-mapped files. */
#endif
